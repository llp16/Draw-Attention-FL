...
def pgd_attack(model, images, labels, criterion, eps=0.3, alpha=0.01, iters=30):
    ...
    for i in range(iters):
        ...
        model.zero_grad()
        cost = criterion(outputs, labels).to(device=device)
        T = torch.sum(cost)
        T.backward()
...
def run_client(idx,clientId, cmodel, iters, learning_rate, dropout_ratio, argdicts = {}):
    ...
    try:
        for itr in range(iters):
            ...
            if args.task == 'nlp':
                ...
                # data = pgd_attack(cmodel, data, target, criterion)
                if clientId in poisoned_clients:
                    data = pgd_attack(cmodel, data, target, criterion)
                end_attack = time.time()
                attack_cost = end_attack - start_attack
                output = cmodel(data)
                loss = criterion(output, target)
    ...
    time_spent = time.time() - run_start - attack_cost
...
def run(rank, model, queue, param_q, stop_flag, client_cfg, poisoned_list=[]):
