...
def init_myprocesses(rank, size, model, queue, param_q, stop_signal, fn, backend, poisoned_list):
    ...
    fn(model, queue, param_q, stop_signal, clientSampler, poisoned_list)
...
def run(model, queue, param_q, stop_signal, clientSampler, poisoned_list):
    ...
    epoch_count = 1
    selected_poisoned_num = 0
    selected_wokers_num = 0
            ...
            try:
                ...
                if len(workersToSend) > 0:          
                    ...
                    if epoch_count % args.resampling_interval == 0 or epoch_count == 2:
                        ...
                        select_poisoned = []
                        for idx in poisoned_list:
                            if idx in clientsToRun:
                                select_poisoned.append(idx)
                        selected_poisoned_num += len(select_poisoned)
                        selected_wokers_num += len(clientsToRun)
                        logging.info("============================================================================================")
                        logging.info("====Try to resample clients, final takes: \n {}"
                                    .format(clientsToRun, ))#virtualClientClock))
                        logging.info("====Selected poisoned clients: \n {} poisoned_ratio={:.2f}%({}/{})"
                                    .format(select_poisoned, 100. * (selected_poisoned_num / selected_wokers_num), selected_poisoned_num, selected_wokers_num))
...
if __name__ == "__main__":
    ...
    for i in range(poisoned_num):
        poisoned_list.append(random.randrange(0, 120))
    ...
    init_myprocesses(this_rank, world_size, model,
                    q, param_q, stop_signal, run, args.backend, poisoned_list
                )
